#(function () {
  ...
  inspect <- function () function (base) binder (callback = function (binder) {
    ...
    with (list (project = function (name, version, packaging) {
              ...
              set = function (key, value) constant (key, value),
  ...
  raven <- function (..., base = getwd (), parent = binder (), restore.libPaths = FALSE)
    binder (parent, function (binder) tryCatch ({
      save <- .libPaths ();
      ...
      inject (function (execution = parent.frame (1))
                          if (length (goals) < 1)
                            goals <<- c (inspect (), fetch (FALSE), import (execution), test (new.env (execution)), install ()),
                        binder);
